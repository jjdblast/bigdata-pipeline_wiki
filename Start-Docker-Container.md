...
* Run the command below including all of the `-p` port mappings
* Contact us at `help@fluxcapacitor.com` with any questions (We'll answer as quickly as we can!)
```
...
```
* Hit `<enter>` a few times for the interactive session to activate (weird, I know)